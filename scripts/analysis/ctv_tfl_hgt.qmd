...
distLongDf <- tidyr::pivot_longer(
  ...
  dplyr::mutate(locus = forcats::fct_relevel(.f = locus, "mash_conserv"))
```
...
The Mash distances are not normally distributed. Therefore, Levene's test for
homogeneity of Mash distance variance across two loci groups (TFL and conserved) is used.

```{r}
car::leveneTest(mash ~ locus, data = distLongDf)
```
