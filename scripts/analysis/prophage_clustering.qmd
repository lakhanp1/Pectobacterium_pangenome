...
phageGroups <- tibble::enframe(
  treeCut, name = "prophage_id", value = "phage_grp"
) %>% 
  dplyr::bind_rows(
    tibble::tibble(
...
across group is selected as a cluster representative.
    ...
    .f = function(x){
      ...
      if (length(x) == 1) {
        ...
        rm <- matrixStats::rowMeans2(subJc, na.rm = TRUE, useNames = TRUE) %>% 
          sort(decreasing = TRUE) %>% round(digits = 3)
...
phageClusters <- dplyr::left_join(
  phageGroups, clusterRoots, by = "prophage_id"
) %>% 
  dplyr::mutate(
    is_root = dplyr::if_else(prophage_id == root_id, 1, 0),
    .after = root_id
  ) 

readr::write_tsv(
  phageClusters, file = confs$analysis$prophages$files$clusters
)
