---
title: "Intra and inter-species ANI score comparison"
date: "`r Sys.time()`"
format: 
  html:
...
Average nucleotide identity (ANI) score distribution within species and between species is compared and visualized in this script.
...
typeStrainAni <- as.data.frame(aniMat) %>%
  ...
  dplyr::left_join(
    y = dplyr::select(sampleInfo, genomeId, sp1 = SpeciesName, type1 = type_material),
    by = c("g1" = "genomeId")
  ) %>%
  dplyr::left_join(
    y = dplyr::select(sampleInfo, genomeId, sp2 = SpeciesName, type2 = type_material),
    by = c("g2" = "genomeId")
  ) %>%
...
## Plot the ANI score distributions
...
ggsave(
  filename = file.path(outDir, "species_ANI_distribution.pdf"),
...
### Compare *P. brasiliense* and *P. versatile* intra-species ANI scores

Comparing means of intra-species ANI scores for *P. brasiliense* and *P. versatile*
using Wilcoxon test. Wilcoxon test is used here because the data is not normally 
distributed. If the data was normally distributed, a t-test would have been used instead.

```{r}
#| echo = FALSE

pt_intraSp <- dplyr::filter(
  aniCmp,
  aniType == "intra-species",
  species %in% c("P. brasiliense", "P. versatile")
) %>% 
  ggplot2::ggplot(
    mapping = aes(x = species, y = ani)
  ) +
  geom_hline(yintercept = 95, color = "black", linetype = "dashed") +
  ggbeeswarm::geom_quasirandom() +
  ggpubr::stat_compare_means(
    label.y.npc = "top", label.x.npc = "center", vjust = -1, hjust = 0.5, size = 6
    ) +
  scale_y_continuous(expand = expansion(add = 1), limits = c(93, 100)) +
  labs(y = "ANI") +
  guides(colour = guide_legend(override.aes = list(size = 5))) +
...
ggsave(
  filename = file.path(outDir, "pbr_pv_ani_distribution.pdf"),
  plot = pt_intraSp, width = 8, height = 8
)
