...
file_regionHaplotypes <- confs$analysis$ctv$files$tape_measure_regions
...
haplotypes <- dplyr::distinct(
  regionHaplotypes, haplotype, hgs
)

# make unique combinations for ctv regions
tailCombs <- tidyr::expand_grid(
  r1 = regionHaplotypes$regionId, r2 = regionHaplotypes$regionId
) %>% 
  dplyr::filter(r1 != r2) %>% 
  dplyr::mutate(
  ...
  dplyr::select(-pair) %>% 
  dplyr::left_join(
    y = dplyr::select(regionHaplotypes, regionId, r1_tail = haplotype),
    by = c("r1" = "regionId")
  ) %>% 
  dplyr::left_join(
    y = dplyr::select(regionHaplotypes, regionId, r2_tail = haplotype),
    by = c("r2" = "regionId")
  ) %>% 
  dplyr::mutate(
...
pt_hap <- dplyr::select(regionHaplotypes, genomeId, haplotype) %>% 
  dplyr::mutate(hap = "y") %>% 
...
pt_all <- pt_hap %>% 
  aplot::insert_left(pt_phy$tree, width = 5)
