...
phageClusters <- suppressMessages(
    ...
    dplyr::select(
        prophage_id, genomeId, phage_grp, fragmented, fragments, nFragments,
        has_prophage, prophage_length, nHg, completeness, taxonomy, is_root, root_id
    )
```
...
cluster_pav_data <- tidyr::expand_grid(
    genomeId = genomesToShow,
    phage_grp = unique(phageClusters$phage_grp)
) %>%
    dplyr::full_join(y = phageClusters, by = c("genomeId", "phage_grp")) %>%
    tidyr::replace_na(list(has_prophage = "n"))

pt_all <- pt_phy$tree +
    scale_x_continuous(expand = expansion(mult = c(0.05, 0.1))) +
    ggnewscale::new_scale_fill() +
    ggtreeExtra::geom_fruit(
        geom = geom_tile, data = cluster_pav_data,
        mapping = aes(y = genomeId, x = phage_grp, fill = has_prophage),
        color = "grey50",
        pwidth = 0.3, width = 0.03, offset = 0.03,
        axis.params = list(axis = "x", text.size = 5, hjust = 1, text.angle = 45)
    ) +
    scale_fill_manual(
        name = NULL,
        values = c("y" = "black", "n" = "white"),
        na.value = alpha("white", 1),
        guide = NULL,
        na.translate = FALSE
    ) +
    ggtreeExtra::geom_fruit(
        mapping = aes(y = id, label = label),
        geom = "geom_text", size = 4, hjust = "left",
