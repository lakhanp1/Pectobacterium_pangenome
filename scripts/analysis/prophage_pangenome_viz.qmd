---
title: "Prophage visualization at the pangenome scale"
date: "`r Sys.time()`"
format: 
  html: 
...
prophageDf <- suppressMessages(readr::read_tsv(confs$data$prophages$files$data)) %>%
    dplyr::select(prophage_id, prophage_length, completeness)

rawTree <- import_tree(file = confs$analysis$phylogeny[[treeMethod]]$files$tree, phylo = TRUE)
...
phageTree <- ape::read.tree(confs$analysis$prophages$files$mash_hclust)

```

## Get prophage presence across the pangenome using homology groups

A function to get homology group counts in each genome of the pangenome
...
## Visualize prophages across the pangenome
