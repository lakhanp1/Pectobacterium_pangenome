...
This script summarizes the consolidated prophages in the pangenome.
...
selectedPro <- dplyr::filter(phages, filtered == 0, nFragments == 1) %>% 
...
removedPro <- dplyr::filter(phages, filtered != 0 | nFragments > 1)

# save filtered prophages
readr::write_tsv(
  dplyr::select(selectedPro, -hgs, -nHgs),
  file = confs$analysis$prophages$preprocessing$files$filtered_prophages
)
...
completenessDf <- dplyr::select(
  selectedPro, prophage_id, genomeId, prophage_length, completeness
) %>% 
  dplyr::mutate(
...
(
  pt_proTax <- dplyr::select(selectedPro, taxonomy) %>% 
    dplyr::mutate(
...
perGenomePhageInfo <- dplyr::group_by(selectedPro, genomeId) %>% 
  dplyr::summarise(
    phage_count = n(),
...
phageSpeciesStats %<>% dplyr::select(
  ...
  phage_nHgs.total, starts_with("phageRatio"),
) %>% 
  dplyr::arrange(desc(n_genomes))
...
rmarkdown::paged_table(
  phageSpeciesStats,
  options = list(
    paged.print = FALSE, cols.print = 10
  )
)
