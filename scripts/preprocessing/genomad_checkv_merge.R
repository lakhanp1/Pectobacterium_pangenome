...
for (sampleId in sampleInfo$sampleId) {
  genomadPath <- paste(confs$data$prophages$path, "/", sampleId, "/", sampleId, sep = "")
  ...
  if (nrow(virSummary) > 0) {
    ...
    vContamination <- suppressMessages(
      ...
      dplyr::select(
        contig_id,
        contamination_types = region_types,
...
prophageDf <- dplyr::select(
  prophageDf,
  contig_id = seq_name, prophage_id, sampleId, SpeciesName, chr, start, end,
...
dplyr::select(prophageDf, sampleId, chr, start, end, prophage_id) %>%
  dplyr::mutate(
    fna = paste(confs$data$prokka$path, "/", sampleId, "/", sampleId, ".fna", sep = ""),
    region = dplyr::if_else(
      is.na(start), true = chr, false = paste(chr, ":", start, "-", end, sep = ""),
    ),
    out = paste(confs$data$prophages$path, "/phage_seqs/", prophage_id, ".fna", sep = ""),
...
dplyr::select(plasmidDf, sampleId, chr, plasmid_id) %>%
  dplyr::mutate(
    fna = paste(confs$data$prokka$path, "/", sampleId, "/", sampleId, ".fna", sep = ""),
    region = chr,
    out = paste(confs$data$plasmids$path, "/plasmid_seqs/", plasmid_id, ".fna", sep = ""),
    faidx = paste("samtools faidx", fna, region, ">", out)
  ) %>%
  dplyr::select(faidx) %>%
  readr::write_tsv(
