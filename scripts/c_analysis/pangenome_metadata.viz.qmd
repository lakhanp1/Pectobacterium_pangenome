...
(
  pt_pbrTimeline <- dplyr::filter(pbrMeta, geo_loc_country == "Netherlands") %>% 
    ggplot2::ggplot() +
    geom_histogram(
...
(
  pt_fnPbr <- dplyr::filter(
    pbrMeta, geo_loc_country == "Netherlands", pbr == "P. brasiliense"
  ) %>% 
    ggplot2::ggplot() +
...
pt_vir <- dplyr::filter(plotDf, virulence %in% c("virulent", "avirulent")) %>% 
  ...
  geom_boxplot(width = .5, outlier.shape = NA, alpha = 0, linewidth = 1) +
  ggbeeswarm::geom_quasirandom(
    mapping = aes(color = virulence), size = 3
  ) +
  ggpubr::stat_compare_means(method = "t.test") +
  scale_color_manual(
    values = c("virulent" = "red", "avirulent" = "green"), na.value = "black"
